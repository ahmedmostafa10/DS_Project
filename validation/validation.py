...
class DataValidationPipeline:
    ...
    def _validate_consistency(self, df):
        ...
        if 'property_type' in df.columns:
            ...
            pairs_to_check = [('apartment', 'apartments'), ('villa', 'villas'), ('townhouse', 'townhouses')]
            mixed_pairs = []
            ...
            if mixed_pairs:
                self.logger.warning(f"[property_type] Found inconsistent singular/plural categories: {', '.join(mixed_pairs)}")
        ...
        if 'offering_type' in df.columns:
            ...
            if len(found_sale) > 1:
                self.logger.warning(f"[offering_type] Found inconsistent 'sale' categories: {', '.join(found_sale)}")
            ...
            if len(found_rent) > 1:
                self.logger.warning(f"[offering_type] Found inconsistent 'rent' categories: {', '.join(found_rent)}")
