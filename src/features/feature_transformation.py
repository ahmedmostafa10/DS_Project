...
class FeatureTransformation:
    ...
    def fit_location_stats(self, df):
        ...
        town_stats = (
            df.groupby("town")
            .agg({"area_value": "mean", "bedrooms": "mean", "bathroom": "mean"})
            .rename(
                columns={
    ...
    def apply_location_stats(self, df, district_stats, town_stats, global_stats):
        ...
        df["district_avg_area"] = (
            df["district_avg_area"].fillna(df["town_avg_area"]).fillna(global_stats["area_value"])
        )
        ...
        df["district_avg_services"] = df["district_avg_services"].fillna(
            global_stats["total_services_count_3km"]
        )
        ...
        df["area_vs_district_avg"] = (df["area_value"] - df["district_avg_area"]) / df[
            "district_avg_area"
        ]
        df["bedrooms_vs_district_avg"] = (df["bedrooms"] - df["district_avg_bedrooms"]) / df[
            "district_avg_bedrooms"
        ]
        df["bathroom_vs_district_avg"] = (df["bathroom"] - df["district_avg_bathroom"]) / df[
            "district_avg_bathroom"
        ]
    ...
    def add_feature_interactions(self, X_train, X_val, X_test):
        ...
        X_train = self.apply_location_stats(
            X_train, self.district_stats, self.town_stats, self.global_stats
        )
        X_val = self.apply_location_stats(
            X_val, self.district_stats, self.town_stats, self.global_stats
        )
        X_test = self.apply_location_stats(
            X_test, self.district_stats, self.town_stats, self.global_stats
        )
    ...
    def select_features(self, X_train, X_val, X_test, y_train):
        selector = VarianceThreshold(threshold=config["feature_selection"]["variance_threshold"])
