...
class DataCollectionPipeline:
    ...
    def get_collection_stats(self):
        ...
        stats = {
            "scraped_records": len(self.scraped_data)
        }
        
        logs_df = pd.DataFrame(self.pipeline_logs)
        if not logs_df.empty:
            stats["logs"] = logs_df.groupby("source_type").agg(
    ...
    def export_all_data(self, output_dir="exports"):
        ...
        if not scraped_df.empty and 'content' in scraped_df.columns:
            ...
            cols_to_drop = [col for col in ['id', 'url', 'title', 'content', 'scraped_at'] if col in scraped_df.columns]
            scraped_df = pd.concat([scraped_df.drop(columns=cols_to_drop), content_expanded], axis=1)
