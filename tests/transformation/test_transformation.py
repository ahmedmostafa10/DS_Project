...
from src.features.transformation_pipeline import DataTransformationPipeline
...
class TestDataTransformationMethods:
    ...
    @pytest.mark.unit
    @pytest.mark.transformation
    def test_transformation_with_valid_data(
        self, transformation_pipeline, sample_transformation_data, temp_dir
    ):
        ...
        transformation_pipeline.run_transformation(
            input_csv=input_csv, output_csv=output_csv, output_report_csv=report_csv
        )
...
class TestFeatureEngineering:
    ...
    def test_categorical_features(self, sample_categorical_data):
        ...
        categorical_cols = sample_categorical_data.select_dtypes(
...
class TestTransformationEdgeCases:
    ...
    def test_single_row_data(self, transformation_pipeline, temp_dir):
        ...
        single_row = pd.DataFrame({"listing_id": ["1"], "bedrooms": [2], "price": [500000]})
    ...
    def test_empty_amenities(self, transformation_pipeline):
        ...
        data = pd.DataFrame(
            {"listing_id": ["1", "2"], "bedrooms": [2, 3], "amenities": ["[]", "[]"]}
        )
