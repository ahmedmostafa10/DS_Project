...
def sample_transformation_data():
    ...
    if csv_file.exists():
        ...
        return pd.DataFrame({
            'city': ['Cairo', 'Giza', 'Alexandria', 'Cairo', 'Giza'],
            'bedrooms': [2, 3, 1, 4, 2],
            'amenities': ['["WiFi", "Parking"]', '["WiFi", "Gym", "Pool"]', '["Parking"]', 
                          '["WiFi", "Parking", "Elevator"]', '["WiFi"]']
        })
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "acquisition: mark test as part of acquisition module"
    )
    config.addinivalue_line(
        "markers", "validation: mark test as part of validation module"
    )
    config.addinivalue_line(
        "markers", "transformation: mark test as part of transformation module"
    )
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        elif "Test" in item.nodeid and not "Test" in item.parent.name:
